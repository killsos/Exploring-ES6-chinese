...
1. Function call: func(···)
2. Dispatched method call: obj.method(···)
3. Direct method call via call(): func.call(···)
4. Direct method call via apply(): func.apply(···)

### 27.2.1 Tail calls in expressions

Arrow functions can have expressions as bodies.

For tail call optimization, we therefore have to figure out where function calls are in tail positions in expressions.

Only the following expressions can contain tail calls:
...
To see why, take a look at the following code, which is equivalent to the previous code:
...
* switch: in its body.
* try-catch: only in the catch clause. The try clause has the catch clause as a context t* can’t be optimized away.
